...
|-----------|------|------------------|
| **1** | Publish backbone (happy path) | Org → XHTML export (headings, paragraphs, bold, italic, links, lists) + `cfl` page update/create wrappers + `org-confluence-publish` command |
...
Current package files live under `packages/org-confluence/`; package-local tests
live under `packages/org-confluence/test/`, with broader repository integration
tests under `test/publishing/confluence/`.

| File | Provides | Responsibility |
|------|----------|----------------|
| `packages/org-confluence/org-confluence.el` | `org-confluence` | Package facade, minor mode, dispatch, and integration entrypoint. |
| `packages/org-confluence/org-confluence-export.el` | `org-confluence-export` | Pure Org → Confluence Storage Format (XHTML) backend. Must not shell out. |
| `packages/org-confluence/org-confluence-api.el` | `org-confluence-api` | `cfl`/REST command construction, configuration lookup, and metadata helpers. |
| `packages/org-confluence/org-confluence-publish.el` | `org-confluence-publish` | Publish/create orchestration, asset upload, and inline-comment preservation. |
| `packages/org-confluence/org-confluence-import.el` | `org-confluence-import` | Confluence Storage XHTML → Org import and page pull. |
| `packages/org-confluence/org-confluence-sync.el` | `org-confluence-sync` | Page sync and page/comment sync orchestration. |
| `packages/org-confluence/org-confluence-comments-*.el` | focused `org-confluence-comments-*` features | Confluence-backed `org-comments` integration. |

## Iteration 1 — Publish Backbone (Happy Path)

**Slice goal:** A human can write an Org buffer with `#+CONFLUENCE_PAGE_ID: <id>`, run `M-x org-confluence-publish`, and see content live on Confluence — with headings, bold, italic, links, and lists rendering correctly.
...
- **Tests to write:**
  - `org-confluence-api--page-update-command` — returns correct shell command string with `--storage` flag
  - `org-confluence-api--page-create-command` — returns correct shell command with `--space`, `--title`, `--storage`
  - `org-confluence-api--page-id-from-buffer` — reads `#+CONFLUENCE_PAGE_ID` from buffer
  - `org-confluence-api--space-from-buffer` — reads `#+CONFLUENCE_SPACE` from buffer
  - `org-confluence-api--page-update-command-with-file` — reads XHTML from a temporary `.xhtml` file via `--file`
  - `org-confluence-api--page-create-command-with-parent` — includes `--parent`
  - `org-confluence-api--page-create-missing-space` — signals error when no space
  - `org-confluence-api--page-update-missing-id` — signals error when no page ID
- **Red signal:** ert errors on undefined functions
- **Green target:** `org-confluence-api.el` with command construction functions. Tests verify command strings without executing them.
...
- **Manual verification:**
  ...
  2. Run `M-x org-confluence-publish`
...
**Slice goal:** A human can write a saved Org buffer with native standalone image links, run `M-x org-confluence-publish`, and see images plus captions rendered on an existing Confluence page.
...
- **Test file:** `test/org-confluence-api-test.el`
- **Tests to write first:**
  - `org-confluence-api--attachment-upload-command` — builds `cfl attachment upload <page-id> <file>`
  - `org-confluence-publish-uploads-images-before-page-edit` — shell commands run upload(s), then page edit
  - `org-confluence-publish-dwim-creates-then-uploads-images` — image documents without `#+CONFLUENCE_PAGE_ID` create the page first, upload attachments to the new page, then update the page content
  - `org-confluence-publish-cleans-temp-xhtml-on-upload-failure` — temp XHTML removed with `unwind-protect`
  - `org-confluence-publish-continues-when-hashed-attachment-exists` — duplicate hashed attachment upload is treated as already uploaded
...
- **Test file:** `test/org-confluence-api-test.el`
- **Tests written first:** raw storage view command construction, basic storage XHTML to Org conversion, nested lists, tables, status chips, emoji fallbacks, panel-like macro callouts, and `org-confluence-pull` opening an imported Org buffer.
...
1. **Storage import canonicalization** — feed representative Confluence storage into `org-confluence-import-storage-to-org`, then assert the canonical Org includes headings, paragraphs, lists, tables, status chips, emoji fallbacks, and callouts.
...
5. **Pull/publish/pull command orchestration** — stub `cfl` command output to prove `org-confluence-pull`, DWIM publish, and a second pull preserve page ID and supported content across command boundaries.
