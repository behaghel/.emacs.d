...
(ert-deftest hub/org-export-hub-article-callout-title-is-explicit ()
  "Hub article callout titles render through class-owned title chrome."
  (let ((tex (org-export-string-as "#+LATEX_CLASS: hub-article\n#+ATTR_CALLOUT: :type warning :title \"Heads up\"\n#+begin_callout\nCareful\n\n- First\n  - Nested\n#+end_callout"
				   'latex t)))
    (should (string-match-p (regexp-quote "\\begin{warning}") tex))
    (should (string-match-p (regexp-quote "\\HubArticleCalloutTitle{Heads up}") tex))
    (should-not (string-match-p (regexp-quote "\\begin{callout}[Heads up]") tex))
    (should (string-match-p "Careful" tex))
    (should (string-match-p (regexp-quote "\\begin{itemize}") tex))))

(ert-deftest hub/org-export-hub-article-horizontal-rule-is-section-break ()
  "Hub article horizontal rules render as class-owned section breaks."
  (let ((tex (org-export-string-as "#+LATEX_CLASS: hub-article\n\nBefore.\n\n-----\n\nAfter."
				   'latex t)))
    (should (string-match-p (regexp-quote "\\begin{HubArticleSectionBreak}") tex))
    (should (string-match-p (regexp-quote "\\end{HubArticleSectionBreak}") tex))
    (should-not (string-match-p (regexp-quote "\\noindent\\rule{\\textwidth}{0.5pt}") tex))))
	...
	(progn
	  ...
	  (with-current-buffer specimen-buffer
	    (hub/test-with-export-compiler-readiness t
							...
							(should (string-match-p (regexp-quote "\\HubArticleMeta{Hubert Behaghel}{May 10, 2026}") tex-contents))
	...
	(progn
	  ...
	  (with-current-buffer specimen-buffer
	    (hub/test-with-export-compiler-readiness t
							...
							(should (string-match-p (regexp-quote "\\pretocmd{\\section}{\\Needspace*{12\\baselineskip}}{}{}") class-contents))
							(should (string-match-p (regexp-quote "\\pretocmd{\\subsection}{\\Needspace*{8\\baselineskip}}{}{}") class-contents))
							(should (string-match-p (regexp-quote "\\RequirePackage{tikz}") class-contents))
							(should (string-match-p (regexp-quote "\\newlength{\\HubArticleOrnamentWeight}") class-contents))
							(should (string-match-p (regexp-quote "\\setlength{\\HubArticleOrnamentWeight}{0.825pt}") class-contents))
							(should (string-match-p (regexp-quote "\\newcommand{\\HubArticleOrnamentDot}") class-contents))
							(should (string-match-p (regexp-quote "circle[radius=\\HubArticleOrnamentDotScale\\HubArticleOrnamentWeight]") class-contents))
							(should (string-match-p (regexp-quote "\\newcommand{\\HubArticleOrnamentRule}") class-contents))
							(should (string-match-p (regexp-quote "\\newcommand{\\HubArticleOrnamentMotif}") class-contents))
							(should (string-match-p (regexp-quote "\\newcommand{\\HubArticleAccentMarker}{\\HubArticleOrnamentMotif{0.825pt}{9mm}{1.8mm}{HubArticleAccentMuted}{1}}") class-contents))
							(should (string-match-p (regexp-quote "\\newcommand{\\HubArticleHeroOrnament}") class-contents))
							(should (string-match-p (regexp-quote "\\HubArticleOrnamentMotif{2.475pt}{27mm}{5.4mm}{HubArticleCalloutRule}{0.8}") class-contents))
							(should (string-match-p (regexp-quote "\\newenvironment{HubArticleSectionBreak}") class-contents))
							...
							(should (string-match-p (regexp-quote "\\RequirePackage{enumitem}") class-contents))
							(should (string-match-p (regexp-quote "\\RequirePackage{tcolorbox}") class-contents))
							(should (string-match-p (regexp-quote "\\tcbuselibrary{breakable}") class-contents))
							(should (string-match-p (regexp-quote "\\definecolor{HubArticleLightCalloutSurface}{HTML}{F3F8F6}") class-contents))
							(should (string-match-p (regexp-quote "\\colorlet{HubArticleCalloutSurface}{HubArticleLightCalloutSurface}") class-contents))
							(should (string-match-p (regexp-quote "\\newcommand{\\HubArticleCalloutTitleColor}{HubArticleAccentMuted}") class-contents))
							(should (string-match-p (regexp-quote "\\textcolor{\\HubArticleCalloutTitleColor}{#1}") class-contents))
							(should (string-match-p (regexp-quote "\\newcommand{\\HubArticleCalloutBullet}") class-contents))
							(should (string-match-p (regexp-quote "\\newcommand{\\HubArticleCalloutNestedBullet}") class-contents))
							(should (string-match-p (regexp-quote "\\setlist[itemize,1]{label=\\HubArticleCalloutBullet,leftmargin=0.88em,labelsep=0.5em,itemsep=0.2em,topsep=0pt") class-contents))
							(should (string-match-p (regexp-quote "\\setlist[itemize,2]{label=\\HubArticleCalloutNestedBullet,leftmargin=0.8em,labelsep=0.45em,itemsep=0.18em,topsep=0pt") class-contents))
							(should (string-match-p (regexp-quote "\\setlist[enumerate,1]{labelindent=0.35em,leftmargin=1.15em,labelsep=0.45em,itemsep=0.2em,topsep=0pt") class-contents))
							(should (string-match-p (regexp-quote "left=2.1em") class-contents))
							(should (string-match-p (regexp-quote "right=2.0em") class-contents))
							(should (string-match-p (regexp-quote "top=1.6em") class-contents))
							(should (string-match-p (regexp-quote "bottom=1.7em") class-contents))
							(should (string-match-p (regexp-quote "leftrule=0.7pt") class-contents))
							(should (string-match-p (regexp-quote "boxrule=0pt") class-contents))
							(should (string-match-p (regexp-quote "sharp corners") class-contents))
