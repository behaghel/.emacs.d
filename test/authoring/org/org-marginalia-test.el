...
(defmacro org-marginalia-test--with-buffer (contents &rest body)
...
(ert-deftest org-marginalia-collects-native-footnote ()
  "A native Org footnote reference becomes a sidenote record."
  (org-marginalia-test--with-buffer "Text[fn:one]\n\n[fn:one] Note body.\n"
				    (let ((notes (org-marginalia-collect)))
...
(ert-deftest org-marginalia-strips-definition-colon-and-metadata ()
...
(ert-deftest org-marginalia-preserves-reference-order ()
  "Marginalia records follow reference order, not definition order."
  (org-marginalia-test--with-buffer "First[fn:b]\nSecond[fn:a]\n\n[fn:a] A.\n\n[fn:b] B.\n"
				    (let ((notes (org-marginalia-collect)))
				      (should (equal '("b" "a") (mapcar (lambda (note) (plist-get note :id)) notes))))))

(ert-deftest org-marginalia-accepts-indented-definitions ()
...
(ert-deftest org-marginalia-layout-pushes-overlaps-down ()
  ...
  (let* ((notes (list (list :id "a" :anchor-line 10 :height 3)
	 ...
	 (laid-out (org-marginalia-layout notes)))
