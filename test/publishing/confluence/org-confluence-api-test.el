  ...
  (let* ((dir (make-temp-file "hub-confluence-inline-marker-index-" t))
	...
	(with-current-buffer (find-file-noselect source)
	  ...
	  (with-temp-buffer
	    ...
	    (should (search-forward ":HUB_COMMENT_REMOTE_MARKER_OCCURRENCE_COUNT: 3" nil t))
	    (should (search-forward ":HUB_COMMENT_REMOTE_MARKER_OCCURRENCE_INDEX: 1" nil t))
	    (should (search-forward ":HUB_COMMENT_TARGET: 59 67" nil t))
	    (should-not (search-forward ":HUB_COMMENT_ANCHOR_STATE: ambiguous" nil t))))
      (when-let* ((buffer (find-buffer-visiting source)))
	(kill-buffer buffer))
      (delete-directory dir t))))

(ert-deftest hub/confluence-comment-import-inline-uses-remote-match-index ()
  ...
  (let* ((dir (make-temp-file "hub-confluence-inline-index-" t))
	...
	(with-current-buffer (find-file-noselect source)
	  ...
	  (with-temp-buffer
	    ...
	    (should (search-forward ":HUB_COMMENT_TARGET_MATCH_INDEX: 1" nil t))
	    (should (search-forward ":HUB_COMMENT_TARGET: 58 65" nil t))
	    (should-not (search-forward ":HUB_COMMENT_ANCHOR_STATE: ambiguous" nil t))))
      (when-let* ((buffer (find-buffer-visiting source)))
	(kill-buffer buffer))
      (delete-directory dir t))))
...
(ert-deftest hub/confluence-image-assets-reuses-missing-hashed-attachment ()
...
(ert-deftest hub/confluence-image-assets-does-not-rehash-existing-hashed-attachment ()
...
(ert-deftest hub/confluence-commands--upload-assets-skips-missing-sources ()
  "Reuse existing Confluence attachments for missing hash-named sources."
  (let ((commands nil))
    (cl-letf (((symbol-function 'hub/confluence-commands--run)
	       (lambda (command) (push command commands))))
