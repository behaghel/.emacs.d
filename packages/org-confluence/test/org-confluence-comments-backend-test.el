  ...
  (let* ((directory (make-temp-file "org-confluence-normalized" t))
	...
	(progn
	  ...
	  (with-temp-file sidecar-file
	    ...
	    (insert ":ORG_COMMENTS_REMOTE_CREATED_AT: 2026-07-02T10:00:00Z\n")
	    (insert ":ORG_COMMENTS_TARGET: 7 20\n")
	    (insert ":ORG_COMMENTS_TARGET_TEXT: selected text\n")
	    (insert ":END:\n\nRemote body.\n")
	    (insert "** OPEN Reply from Bob\n")
	    (insert ":PROPERTIES:\n")
	    (insert ":ORG_COMMENTS_ID: remote-confluence-r1\n")
	    (insert ":ORG_COMMENTS_SOURCE: confluence\n")
	    (insert ":ORG_COMMENTS_SYNC_KIND: reply\n")
	    (insert ":ORG_COMMENTS_REMOTE_ID: r1\n")
	    (insert ":ORG_COMMENTS_REMOTE_PARENT_ID: c1\n")
	    (insert ":ORG_COMMENTS_REMOTE_AUTHOR_DISPLAY_NAME: Bob\n")
	    (insert ":ORG_COMMENTS_REMOTE_CREATED_AT: 2026-07-02T10:05:00Z\n")
	    (insert ":END:\n\nReply body.\n"))
	  (with-current-buffer (find-file-noselect source-file)
	    ...
	    (let* ((comment (car (org-comments-collect (current-buffer) t)))
	      ...
	      (should (eq (plist-get reply :backend) 'confluence))
	      (should (eq (plist-get reply :remote-state) 'present))
	      (should (equal (plist-get reply :sync-kind) "reply"))
	      (should (equal (plist-get reply :remote-author-name) "Bob"))
