...
(defvar org-comments-panel-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "d") #'org-comments-delete)
    (define-key map (kbd "e") #'org-comments-edit)
    (define-key map (kbd "g") #'org-comments-panel-refresh)
    (define-key map (kbd "m") org-comments-panel-status-map)
    (define-key map (kbd "D") #'org-comments-pull)
    (define-key map (kbd "O") #'org-comments-open-remote)
    (define-key map (kbd "o") #'org-comments-open-remote)
    (define-key map (kbd "p") #'org-comments-page-open-at-point)
    (define-key map (kbd "S") #'org-comments-sync)
    (define-key map (kbd "U") #'org-comments-push)
    (define-key map (kbd "q") #'org-comments-close-current-ui)
    (define-key map (kbd "r") #'org-comments-reply)
