...
`:skipped-resolved', and `:preserved-local' keys."
  (let* ((provider (or (plist-get report :provider) "Remote"))
    ...
    (let ((remote-reopened (org-comments-import-report--count report :remote-reopened)))
      (when (> remote-reopened 0)
	(setq parts (append parts (list (format "remote reopened %s" remote-reopened))))))
    ...
    (let ((remote-present (org-comments-import-report--count report :remote-present)))
      (when (> remote-present 0)
	(setq parts (append parts (list (format "remote present %s" remote-present))))))
...
(defun org-comments-sync-report-format (report)
  ...
  (let* ((provider (or (plist-get report :provider) "Remote"))
	 (parts nil))
    (dolist (spec '((:created . "created")
		    (:updated . "updated")
		    (:pushed . "pushed")
		    (:pushed-replies . "pushed replies")
		    (:resolved . "resolved")
    ...
    (concat provider " comments: " (if parts (string-join parts ", ") "no changes")
	    (when (plist-get report :url-copied) "; URL copied"))))
