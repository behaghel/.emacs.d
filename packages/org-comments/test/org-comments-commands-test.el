...
(ert-deftest org-comments-commands-default-prefix-is-c-c-control-semicolon ()
  "The default minor-mode prefix avoids Org mode's `C-c ;' binding."
  (should (equal org-comments-keymap-prefix "C-c C-;"))
  (should-not (lookup-key org-comments-mode-map (kbd "C-c ;")))
  (should (eq (lookup-key org-comments-mode-map (kbd "C-c C-; c"))
	      #'org-comments-create))
  (should (eq (lookup-key org-comments-mode-map (kbd "C-c C-; P"))
	      #'org-comments-page-create))
  (should (eq (lookup-key org-comments-mode-map (kbd "C-c C-; n"))
	      #'org-comments-next))
  (should (eq (lookup-key org-comments-mode-map (kbd "C-c C-; p"))
  ...
  (should (eq (lookup-key org-comments-mode-map (kbd "C-c C-; U"))
	      #'org-comments-push))
  (should (eq (lookup-key org-comments-mode-map (kbd "C-c C-; D"))
	      #'org-comments-pull))
  (should (eq (lookup-key org-comments-mode-map (kbd "C-c C-; m r"))
  ...
  (let ((org-comments-keymap-prefix "C-c '"))
	...
	(progn
	  ...
	  (should-not (eq (lookup-key org-comments-mode-map (kbd "C-c ; c"))
			  #'org-comments-create)))
      (let ((org-comments-keymap-prefix "C-c C-;"))
	(org-comments-rebuild-mode-map)))))
