...
- `org-google-docs-footnotes.el` extracts a native-footnote push plan for named Org footnotes before any Google Docs mutation, wires the `gdocs` footnote-reference seam into `org-google-docs-push`, and provides two-phase request helpers for `createFootnote` followed by body insertion into returned footnote segments.
