...
| Callout target | Preserve canonical `#+BEGIN_CALLOUT` blocks with `#+ATTR_CALLOUT:` metadata | V1 uses semantic `callout-block` IR with `:callout-type`, optional `:title`, and paragraph body runs. Google Docs renders a generated visible label paragraph (`Warning — Heads up`) plus editable body paragraphs. Label/body named-range markers store type/title metadata; pull drops the generated label as chrome and preserves remote body edits. Generated empty separator paragraphs are callout chrome and are included in pulled block ranges so repeated push/restyle operations do not accumulate blank paragraphs. Missing type defaults to `info`; unsupported types fail closed. |
