...
| Quote target | Preserve quote-block boundaries on round trip using semantic markers and logical block-line styles | Google Docs has no stable native quote-block object in the public Docs API. A marker-backed semantic block preserves Org meaning while authoring-owned paragraph styles provide indentation, background shading, spacing, or future borders. Avoid quote text-run styles such as italic/foreground color because they pull back as Org content formatting and create merge noise. |
| Callout target | Preserve canonical `#+BEGIN_CALLOUT` blocks with `#+ATTR_CALLOUT:` metadata | V1 uses semantic `callout-block` IR with `:callout-type`, optional `:title`, and generic body paragraph elements, including unordered, ordered, and nested list items using the same paragraph/list IR foundations as top-level content. Google Docs renders a generated visible label paragraph (`Warning — Heads up`) plus editable body paragraphs/list items. Label/body named-range markers store type/title metadata; pull drops the generated label as chrome and preserves remote body edits. Generated empty separator paragraphs are callout chrome and are included in pulled block ranges so repeated push/restyle operations do not accumulate blank paragraphs. Missing type defaults to `info`. Callout composition is limited to paragraphs/lists; nested blocks such as tables, source blocks, and quote blocks fail closed because blocks do not compose inside blocks. |
...
- [x] AC-11a: Given a canonical Org callout (`#+ATTR_CALLOUT:` + `#+BEGIN_CALLOUT`), when pushed and pulled, then type, optional title, body paragraphs, and inline body formatting survive via semantic callout IR and label/body named-range markers.
...
| AC-11a | Org → IR/request and IR/JSON → Org callout fixture tests; live smoke checklist to validate generated label/body behavior | Yes |
