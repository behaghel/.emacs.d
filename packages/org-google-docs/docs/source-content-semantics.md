...
- [x] AC-3: Given a Google Doc with native footnotes, when converted by the local `gdocs` pull seam, then the Org buffer contains corresponding ordinary footnote references and definitions without losing surrounding paragraph text.
