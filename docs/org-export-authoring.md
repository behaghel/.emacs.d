...
| Key | Action |
|-----|--------|
| `C` | Publish/create DWIM for the current buffer or selected subtree |
| `O` | Publish/create DWIM and open the resulting Confluence page |
| `X` | Export to a temporary Confluence XHTML buffer |
| `x` | Export to a `.xhtml` file |

For whole-buffer updates, set `#+CONFLUENCE_PAGE_ID:`. For subtree updates, put `CONFLUENCE_PAGE_ID` in the subtree property drawer and use Org's subtree export option from the dispatcher. In `C-c C-e C`, `C` publishes DWIM: it updates when an ID exists and creates otherwise. `O` publishes DWIM and opens the resulting Confluence page using `org-confluence-api-base-url`. For create flow, set `#+CONFLUENCE_SPACE:` or configure `org-confluence-api-default-space` in your integration config; after a successful create, the buffer is updated with `#+CONFLUENCE_PAGE_ID:` and `#+CONFLUENCE_SPACE:` for future publishes, but is not auto-saved.

Use `M-x org-confluence-pull` to fetch a Confluence page by ID into a new Org buffer. Pull currently performs a conservative import of common storage XHTML: headings, paragraphs, inline emphasis/code/links, ordered/unordered lists including sublists, tables, Confluence status chips as `[[confluence-status:Colour][Text]]` links, Confluence emoji as Unicode fallback text, and panel-like macros as semantic callouts.

Manual acceptance for the DWIM publish/create/open flow has been completed. Known future fidelity work is driven by real pages that fail round-trip expectations. If complex Confluence-only structures need preservation, prefer a raw preservation block named `#+begin_confluence_markup` / `#+end_confluence_markup` rather than flattening content prematurely.
